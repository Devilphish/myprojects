...
int eval_print_hand(struct card *d, int n)
{
    int vals[14];
    int total;
    int str8;
    ...
    if (summary == 0) {
        ...
        /* exploit fact that there is no pair or set */
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        printf("usage: %s <num_loops> <num_stacks>\n", argv[0]);
        exit(1);
    }
    ...
    hand = malloc(3 * sizeof (*hand));

    for (i = 0; i < 52; i++) {
        deck[i] = cards[i];
    }

    highcard = pair = twopair = set = straight = flush = 0;
    fullboat = quads = straightflush = 0;

    for (j = 0; j < nloops; j++) {
        shuffle(deck, ncards);
        for (k = 0; k < nstacks; k++) {
            d = &deck[52 - ncards + (k * 3)];
            print_deck(d, 3);
            printf("-  "); 
            eval_print_hand(d, 3);
        }
